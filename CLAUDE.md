...
5. **Verify before claiming.** Run `npm run build` (type-check + build). If
   ...
   Performance is part of correct here, because the league reads this on
   phones: nothing third-party on the critical path (the three faces are
   self-hosted in `public/fonts`, regenerated by `scripts/fonts.mjs` and
