 ...
 * seamlessly at -50%; hovering pauses it so a line can actually be read, and
 * a tap does the same on a phone.
 *
 * Held still — reduced motion with the FX toggle off — the tape is a
 * scrollable strip instead, its duplicate hidden by the guard in index.css.
 * There is nothing to pause then, so it drops the button and says what it
 * really is.
 */
export default function Ticker({ trades }: { trades: Trade[] }) {
  ...
  const handles: HTMLAttributes<HTMLDivElement> = still
    ? {
        role: 'group',
        tabIndex: 0,
        'aria-label': 'Transaction tape — scroll to read',
      }
    : {
        role: 'button',
        tabIndex: 0,
        'aria-pressed': paused,
        'aria-label': paused
          ? 'Transaction tape, paused — tap to resume'
          : 'Transaction tape — tap to pause',
